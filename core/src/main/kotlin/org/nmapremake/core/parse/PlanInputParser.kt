...
object PlanInputParser {
    ...
    fun parse(input: Input): ParseOutcome {
        val target = when (val outcome = TargetParser.parse(input.target)) {
        ...
        val tcpPorts: PortSpec? = if (portsText.isEmpty()) {
            null
        } else {
    ...
    private fun entriesToSpec(entries: List<PortSpecParser.PortEntry>): PortSpec {
        ...
        val ports = entries.flatMap { entry ->
            when (entry) {
    ...
    sealed interface ParseOutcome {
        data class Success(val plan: ScanPlan) : ParseOutcome
