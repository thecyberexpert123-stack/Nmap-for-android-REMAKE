...
data class CapabilityProfile(
    ...
    fun availability(capability: Capability): Availability = entries[capability] ?: Availability.UNKNOWN

    /** True only when the capability is actually executable (SUPPORTED or LIMITED). */
    fun supports(capability: Capability): Boolean = availability(capability) in EXECUTABLE
    ...
    companion object {
        ...
        val ANDROID_LOCAL_M1: CapabilityProfile =
            CapabilityProfile(
                mapOf(
