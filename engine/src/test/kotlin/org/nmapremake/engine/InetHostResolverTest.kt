...
class InetHostResolverTest {
    ...
    @Test
    fun `IPv4 literal resolves to itself`() = runTest {
    ...
    @Test
    fun `IPv6 literal resolves to itself`() = runTest {
    ...
    @Test
    fun `localhost resolves with a family-consistent address`() = runTest {
    ...
    @Test
    fun `reserved invalid TLD raises UNRESOLVABLE_HOST`() = runTest {
        // RFC 2606 reserves ".invalid" — guaranteed not to resolve.
        val exception = assertFailsWith<UnresolvedHostException> {
